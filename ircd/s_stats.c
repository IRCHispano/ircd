...
static void stats_klines(struct Client *sptr, const struct StatDesc *sd,
		char *mask);
static void stats_elines(struct Client *sptr, const struct StatDesc *sd,
        char *mask);
...
static struct StatDesc statsinfo[] = {
  { 'a', "nameservers", STAT_FLAG_OPERFEAT|STAT_FLAG_LOCONLY, FEAT_HIS_STATS_NAMESERVERS,
    report_dns_servers, 0,
    "DNS servers." },
#if defined(DDB)
  { 'b', "DDB", STAT_FLAG_OPERFEAT, FEAT_HIS_STATS_DDB,
    0, 0, //ddb_report_stats, 0,
  ...
  { 'd', "maskrules", (STAT_FLAG_OPERFEAT | STAT_FLAG_CASESENS), FEAT_HIS_STATS_CRULES,
    stats_crule_list, CRULE_MASK,
    "Dynamic routing configuration." },
  { 'D', "crules", (STAT_FLAG_OPERFEAT | STAT_FLAG_CASESENS), FEAT_HIS_STATS_CRULES,
  ...
  { 'R', "mappings", (STAT_FLAG_OPERFEAT | STAT_FLAG_CASESENS), FEAT_HIS_STATS_MAPPINGS,
    stats_mapping, 0,
    "Service mappings." },
#ifdef DEBUGMODE
...
#if defined(DDB)
  for (ddb = ddb_iterator_first(DDB_EXCEPTIONDB); ddb;
       ddb = ddb_iterator_next()) {
    send_reply(to, RPL_STATSELINE, "Ddb", ddb_content(ddb));
  }
#endif
}
    ...
    for (i = 0; i <= HighestFd; i++)
    {
      ...
      send_reply(sptr, SND_EXPLICIT | RPL_STATSLINKINFO,
            ...
            acptr->cli_connect->comp_in_total_out) + 0.5) : 100,
                 ...
                 100,
#endif
                 CurrentTime - cli_firsttime(acptr));
    }
}
