/*
 * IRC-Dev IRCD - An advanced and innovative IRC Daemon, ircd/channel.c
 *
 ...
 *
 * @returns True if the user is a chan owner (And not a zombie), False otherwise.
 * @see \ref zombie
 */
int is_chan_owner(struct Client *cptr, struct Channel *chptr)
{
  struct Membership* member;
  assert(chptr);
  if ((member = find_member_link(chptr, cptr)))
    return (!IsZombie(member) && IsChanOwner(member));
...
int client_can_send_to_channel(struct Client *cptr, struct Channel *chptr, int reveal)
{
   ...
   */
#if defined(DDB) || defined(SERVICES)
  if (IsServer(cptr) || IsChannelService(cptr) || IsServicesBot(cptr))
#else
  if (IsServer(cptr))
#endif
    return 1;
...
struct Channel *get_channel(struct Client *cptr, char *chname, ChannelGetType flag)
{
  ...
  len = strlen(chname);
#if defined(DDB)
  if ((!cptr || MyUser(cptr)) && len > CHANNELLEN)
#else
...
#if defined(DDB)
    chptr->creationtime = (!cptr || MyUser(cptr)) ? TStime() : (time_t) 0;
#else
...
/*  MODE_UPASS,		'U', */
  ...
  for (i = 0; i < mbuf->mb_count; i++) {
...
#if defined(DDB) || defined(SERVICES)
    if (MB_TYPE(mbuf, i) & (MODE_OWNER | MODE_CHANOP | MODE_VOICE)) {
#else
	...
	MB_TYPE(mbuf, i) |= MODE_SAVE; /* save for later */
        ...
        switch(MB_TYPE(mbuf, i) & (MODE_OWNER | MODE_CHANOP | MODE_VOICE))
        {
          case MODE_OWNER:
            mode_char = 'q';
            break;
  ...
  if (mbuf->mb_dest & MODEBUF_DEST_SERVER) {
    ...
    if (mbuf->mb_dest & MODEBUF_DEST_OPMODE) {
      ...
      sendcmdto_one(&me, CMD_MODE, mbuf->mb_connect, "%H %s%s%s%s%s%s %Tu",
		    ...
		    mbuf->mb_channel->creationtime);
#if defined(DDB)
    } else if (mbuf->mb_dest & MODEBUF_DEST_BOTMODE) {
        sendcmdto_serv(mbuf->mb_source, CMD_BMODE, mbuf->mb_connect,
                       "%s %H %s%s%s%s%s%s %Tu", mbuf->mb_botname, mbuf->mb_channel,
                       rembuf_i ? "-" : "", rembuf, addbuf_i ? "+" : "",
                       addbuf, remstr, addstr,
                       mbuf->mb_channel->creationtime);
#endif /* defined(DDB) */
...
#if defined(DDB) || defined(SERVICES)
                           (MODE_OWNER |MODE_CHANOP | MODE_VOICE));
#else
	   ...
	   struct Membership* member)
{
  ...
  while (*modestr) {
	...
	break;
      ...
      case 'z': /* SSL */
        if (((MyConnect(sptr) && IsSSL(sptr))
            || !MyConnect(sptr)) || IsServer(sptr))
...
# endif
      case 'q':
        if (!IsServer(cptr) || (state.dir == MODE_ADD &&
            !(mbuf->mb_dest & MODEBUF_DEST_BOTMODE)))
         /* state.flags |= MODE_PARSE_NOTADD; */
          break;
...
joinbuf_join(struct JoinBuf *jbuf, struct Channel *chan, unsigned int flags)
{
  ...
  if (jbuf->jb_type == JOINBUF_TYPE_PART ||
	...
	sendcmdto_channel((chan->mode.apass[0] ? &his : jbuf->jb_source),
      ...
      if (flags & CHFL_OWNER)
      {
        char *botname = ddb_get_botname(DDB_CHANSERV);

        sendcmdbotto_channel(botname, CMD_MODE, chan, NULL, 0,
                             "%H +q %C", chan, jbuf->jb_source);
      }
      else if (flags & CHFL_CHANOP)
        sendcmdto_channel(&me, CMD_MODE, chan, NULL, SKIP_SERVERS,
                          "%H +o %C", chan, jbuf->jb_source);
#else
